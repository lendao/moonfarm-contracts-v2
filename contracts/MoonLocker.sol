...
contract MoonLocker is Ownable, ReentrancyGuard {
  ...
  constructor(IUniFactory _uniswapFactory) {
    devaddr = payable(address(msg.sender));
    gFees.referralPercent = 250; // 25%
    gFees.ethFee = 100000000000000000; // 0.1
    gFees.secondaryTokenFee = 500000000000000000; // 0.5 tokens
    gFees.secondaryTokenDiscount = 400; // 40%
    gFees.liquidityFee = 10; // 1%
    gFees.referralHold = 5e18; // 5 tokens
